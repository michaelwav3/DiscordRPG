...
function craft(message, userData) {
    ...
    const recipe = recipes_1.RECIPES.find(r => r.id === recipeId);
    if (!recipe) {
        //@ts-ignore
    ...
    for (const [itemId, needed] of Object.entries(recipe.inputs)) {
        ...
        const owned = user.inventory.filter((i) => i === itemId).length;
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .setDescription(
    //@ts-ignore
    `You crafted **${totalOutput}x ${items_1.ITEMS[itemId]?.name ?? itemId}**\n\n` +
        `**Times crafted:** ${amount}`)
        .setColor(0x57f287);
    //@ts-ignore
    message.channel.send({ embeds: [embed] });
}
function craftList(message) {
    const embed = new discord_js_1.EmbedBuilder()
        .setTitle("📜 Crafting Recipes")
        .setDescription(recipes_1.RECIPES.map(recipe => {
        const requirements = Object.entries(recipe.inputs)
            .map(([itemId, qty]) => {
            const itemName = items_1.ITEMS[itemId]?.name ?? itemId;
            return `${qty}x ${itemName}`;
        })
            .join(", ");
        const outputName = items_1.ITEMS[recipe.output.itemId]?.name ?? recipe.output.itemId;
