...
export async function blackjack(message: Message, userData: any) {
  ...
  function buildEmbed(showDealerHole: boolean = false, footerText?: string) {
  ...
  const filter = (i: any) =>
    i.user.id === message.author.id && i.message.id === gameMessage.id;
