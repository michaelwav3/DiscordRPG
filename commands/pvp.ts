...
import fs from "fs";
import { ITEMS, ItemDef } from "../data/items"; // note the ../ and include ItemDef interface

// ===================================================================
// ================  IMPORTED / COPIED COMBAT LOGIC  =================
// ===================================================================
 ...
 */
function processAbilityEffects(actor: any, target: any, ability: ItemDef, damageCalculator: (actor: any, ability: any) => number): string {
  let log = "";
  ...
  for (const type of effectsToProcess) {
    ...
    if (type === "attack" && ability.damageAmount && ability.damageAmount > 0) {
      if (isSelfTarget) continue; // Attack effect must target opponent in PvP
...
export async function startPvP(message: Message, userData: any) {
  ...
  if (challengerUser.location !== opponentUser.location) {
    ...
    return message.channel.send(
      `You must both be in the same location to duel!\n` +
  ...
  if (args.length > 1) {
    ...
    if (!isNaN(maybeNumber) && maybeNumber > 0) wager = maybeNumber;
  }
  if (wager < 0) wager = 0;
  ...
  if (wager > 0) {
    if ((challengerUser.balance || 0) < wager) {
      //@ts-ignore
      return message.channel.send(
        `${message.author.username}, you don't have enough coins for a **${wager}** coin wager.`
      );
    }
    if ((opponentUser.balance || 0) < wager) {
      //@ts-ignore
      return message.channel.send(
        `${mentioned.username} doesn't have enough coins for a **${wager}** coin wager.`
      );
    }
  }
  ...
  const challengeMessage = await message.channel.send({
    content:
      `⚔️ **${message.author.username}** has challenged **${mentioned.username}** to a duel!` +
  ...
  const filter = (i: any) =>
    i.user.id === opponentId && i.message.id === challengeMessage.id;
  ...
  collector.on("collect", async (interaction: any) => {
    if (interaction.customId === "pvp_accept") {
      ...
      await interaction.update({
        content:
          `✅ **${mentioned.username}** accepted the duel against **${message.author.username}**!` +
          (wager > 0 ? `\n💰 Wager: **${wager} coins** each` : ""),
        components: [],
      });
...
async function runDuel(
  ...
  const opponentName =
    message.guild?.members.cache.get(opponentId)?.user.username ||
    "Opponent";
  ...
  if (challengerUser.location !== opponentUser.location) {
    ...
    await message.channel.send(
      "Duel cancelled: you are no longer in the same location."
    );
    return;
  }
  ...
  if (wager > 0) {
    if ((challengerUser.balance || 0) < wager) {
      //@ts-ignore
      await message.channel.send(
        `${challengerName} no longer has enough coins for the wager. Duel cancelled.`
      );
      return;
    }
    if ((opponentUser.balance || 0) < wager) {
      //@ts-ignore
      await message.channel.send(
        `${opponentName} no longer has enough coins for the wager. Duel cancelled.`
      );
      return;
    }
  }
  ...
  const p1 = {
    ...
    statuses: [] as StatusEffect[], // 🚩 ADDED STATUS TRACKING
  ...
  function calcSkillDamage(actor: any, ability: any): number {
  ...
  function calcSpellDamage(actor: any, ability: any): number {
  ...
  const filter = (i: any) =>
    [p1.id, p2.id].includes(i.user.id) && i.message.id === duelMessage.id;
  ...
  function getTarget(actorId: string) {
  ...
  async function advanceTurn(interaction: any, previousLog: string) {
