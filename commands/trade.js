...
const activeTrades = new Map(); // key: tradeMessageId
...
function buildTradeEmbed(session, userData) {
    ...
    const listItems = (ids) => ids.length === 0
        ? "_None_"
        : ids
            .map((id) => {
            const def = items_1.ITEMS[id];
...
// ---------- Main entry ----------
async function trade(message, userData) {
    ...
    if (!session) {
        //@ts-ignore
        return message.channel.send("You are not currently in an active trade.");
    }
    if (sub === "add" || sub === "remove") {
        ...
        if (kind === "item") {
            ...
            if (!itemId) {
                //@ts-ignore
                return message.channel.send("Specify an item ID. Example: `%trade add item wooden_stick`");
            }
            ...
            if (!amount || amount <= 0) {
                //@ts-ignore
                return message.channel.send("Specify a positive coin amount. Example: `%trade add coins 50`");
            }
            return modifyTradeCoins(message, userData, session, mode, amount);
        }
        else {
            //@ts-ignore
            return message.channel.send("Unknown trade subcommand. Use `item` or `coins`.\nExample: `%trade add item wooden_stick`");
        }
    }
    //@ts-ignore
    return message.channel.send("Unknown trade command. Valid examples:\n" +
        "`%trade @user`\n" +
        "`%trade add item <itemId>`\n" +
        "`%trade remove item <itemId>`\n" +
        "`%trade add coins <amount>`\n" +
...
async function startTradeRequest(message, userData, args) {
    const sender = userData[message.author.id];
    ...
    if (findSessionForUser(message.author.id) || findSessionForUser(targetUser.id)) {
        //@ts-ignore
        return message.channel.send("Either you or that user is already in an active trade.");
    }
    ...
    const filter = (i) => i.user.id === targetUser.id && i.message.id === reqMsg.id;
...
async function startTradeSession(message, userData, user1Id, user2Id) {
    const session = {
    ...
    const tradeMsg = await message.channel.send({
        content: `📦 Trade started between <@${user1Id}> and <@${user2Id}>.\n` +
            `Use:\n` +
    ...
    const filter = (i) => [user1Id, user2Id].includes(i.user.id) && i.message.id === tradeMsg.id;
...
async function modifyTradeItem(message, userData, session, mode, itemId) {
    ...
    if (mode === "add") {
        ...
        if (available <= 0) {
            //@ts-ignore
            return message.channel.send("You don't have any more of that item to offer.");
        }
        ...
        await updateTradeWindow(message, userData, session);
        //@ts-ignore
        return message.channel.send(`Removed **${itemId}** from your trade offer.`);
    }
}
// ---------- Modify coins in an active trade ----------
async function modifyTradeCoins(message, userData, session, mode, amount) {
    const userId = message.author.id;
    const offer = session.offers[userId];
    if (!offer) {
        //@ts-ignore
    ...
    if (mode === "add") {
        if (offer.coins + amount > balance) {
            //@ts-ignore
            return message.channel.send("You can't offer more coins than you currently have.");
        }
        offer.coins += amount;
    }
    else {
        if (offer.coins < amount) {
            //@ts-ignore
            return message.channel.send("You can't remove more coins than you have offered.");
        }
        offer.coins -= amount;
    }
    //@ts-ignore
    ...
    await updateTradeWindow(message, userData, session);
    //@ts-ignore
    return message.channel.send(`Your coin offer is now **${offer.coins}** coins.`);
}
// ---------- Update trade window embed ----------
async function updateTradeWindow(message, userData, session) {
    try {
        const tradeMsg = await message.channel.messages.fetch(session.messageId);
        await tradeMsg.edit({
            content: tradeMsg.content,
            embeds: [buildTradeEmbed(session, userData)],
            components: tradeMsg.components,
        });
    }
    catch {
        // ignore
...
async function finalizeTrade(message, userData, session) {
    const u1 = userData[session.user1Id];
    const u2 = userData[session.user2Id];
    if (!u1 || !u2) {
        //@ts-ignore
        await message.channel.send("❌ Trade failed: one of the users is no longer registered.");
        return false;
    }
    const o1 = session.offers[session.user1Id];
    const o2 = session.offers[session.user2Id];
    // Re-validate items & coins for both sides
    const hasEnough = (user, offer) => {
        ...
        for (const [id, cnt] of Object.entries(neededCounts)) {
            if (!invCounts[id] || invCounts[id] < cnt)
                return false;
        }
        if ((user.balance || 0) < offer.coins)
            return false;
        return true;
    };
    //@ts-ignore
    if (!hasEnough(u1, o1) || !hasEnough(u2, o2)) {
        //@ts-ignore
        await message.channel.send("❌ Trade failed: one of the users no longer has the offered items/coins.");
        return false;
    }
    ...
    if (!Array.isArray(u1.inventory))
        u1.inventory = [];
    if (!Array.isArray(u2.inventory))
        u2.inventory = [];
    //@ts-ignore
