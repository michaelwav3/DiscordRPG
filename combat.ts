...
import fs from "fs";
import { incrementKillQuests } from "./utils/questHelpers";
...
// NOTE: giveItem is not used here but is called inside the new helper function.

export interface EnemyDrop {
  itemId: string;   // must exist in ITEMS
...
type StatusType = "burn" | "poison" | "paralyze" | "freeze";

interface StatusEffect {
  type: StatusType;
  duration: number;
  potency?: number;   // damage or chance
}
 ...
 */
function processMumboVictory(user: any, enemyName: string): string {
    let log = "";
    ...
    const element = user.element as keyof typeof ELEMENTAL_MOVES;
...
if (user.HP > user.vitality*10) {user.HP = user.vitality * 10};
...
async function endCombat() {
  ...
  activeCombats.delete(user.location);
  ...
  activeCombats.set(user.location, {
  party,
  ...
  let turnIndex = 0;
let playerTurnsRemaining: number[] = [];
let enemyTurnsRemaining = 1;
  ...
  function calcPhysicalDamage(str: number) {
    const base = str * 2;
  ...
  function calcSpellDamage(actor: any, ability: any) {
  ...
  function applyStatus(target: any, status: StatusEffect) {
  const existing = target.statuses.find((s: StatusEffect) => s.type === status.type);
...
function processStatuses(target: any): { skipTurn: boolean; log: string } {
  let log = "";
  ...
  for (const status of [...target.statuses]) {
    ...
    if (status.duration <= 0) {
      target.statuses = target.statuses.filter((s: StatusEffect) => s !== status);
 ...
 function startNewRound() {
  ...
  const avgPartySpeed =
    partySpeeds.reduce((a, b) => a + b, 0) / partySpeeds.length || 1;

  playerTurnsRemaining = party.map(p =>
    Math.max(1, Math.min(10, Math.floor(getSpeed(p) / Math.max(1, enemy.speed))))
  );

  enemyTurnsRemaining = Math.max(
    1,
    Math.min(10, Math.floor(enemy.speed / avgPartySpeed))
  );
  ...
  function rollDrops(): string[] {
  const obtainedItemIds: string[] = [];
  const drops = enemyConfig.drops;
  if (!drops || drops.length === 0) return obtainedItemIds;

  for (const drop of drops) {
    const rolls =
      drop.min && drop.max ? randInt(drop.min, drop.max) : 1;
  ...
  async function handleVictory(interaction: any, log: string) {
    combatOver = true;
    await endCombat();
...
if (partySize === 1) {
  ...
  coinText =
    ...
    // quest kill tracking
    incrementKillQuests(user, enemy.name.toLowerCase());
 ...
 */
function advanceTurn(): "player" | "enemy" {
  //@ts-ignore
 ...
 */
function processAbilityEffects(actor: any, primaryTarget: any, ability: ItemDef, damageCalculator: (actor: any, ability: any) => number): string {
  let log = "";
  
  // Safely convert abilityType (string or array) into an array of effects
  const effectsToProcess: (string | undefined)[] = Array.isArray(ability.abilityType) 
    ? ability.abilityType 
  ...
  for (const type of effectsToProcess) {
    ...
    if (type === "attack" && ability.damageAmount && ability.damageAmount > 0) {
      ...
      if (targetForDamage.hp < 0) targetForDamage.hp = 0;
...
startNewRound();
  ...
  function enemyActOnce(): string {
    ...
    // Try skill first
    if (
      enemy.skillIds.length > 0 &&
      roll < enemy.skillChance &&
      enemy.sp > 0
    ) {
      const skillId =
        enemy.skillIds[Math.floor(Math.random() * enemy.skillIds.length)];
      //@ts-ignore
      const sk: ItemDef = ITEMS[skillId];
      if (sk && sk.type === "skill") {
    ...
    if (
      enemy.spellIds.length > 0 &&
      roll2 < enemy.spellChance &&
      enemy.mp > 0
    ) {
      const spellId =
        enemy.spellIds[Math.floor(Math.random() * enemy.spellIds.length)];
      //@ts-ignore
      const sp: ItemDef = ITEMS[spellId];
      if (sp && sp.type === "spell") {
    ...
    const finalDmg = Math.max(
      1,
      Math.floor(rawDmg * (target.defenseMultiplier ?? 1))
    );
...
if (enemy.hp <= 0) break;
if (enemyStatus.skipTurn) continue;
...
const dmg = calcPhysicalDamage(currentPlayer.str + attackBonus);
    ...
    // ---------- DEFEND ----------
      ...
      const log =
          `🛡️ **${currentPlayer.name}** braces for impact!\n` +
  ...
  enemy.fleeResist
      ...
      if (roll < chance) {
        ...
        const failText =
          `❌ **${currentPlayer.name}** tried to flee but **${enemy.name}** blocked the escape! (Chance: ${(chance * 100).toFixed(1)}%)`;
